...
def clip_model_to_regions(
    ...
    with progress_bar_context(
        ...
        if not regions_file:
            # Check if the regions file exists on the local file system (for editable installations)
            local_regions_file = os.path.join(
                os.path.dirname(__file__), "tide_model", "tide_regions_map.geojson"
            )
            if os.path.exists(local_regions_file):
                regions_file = local_regions_file
                logger.info(f"Using local regions file at {regions_file}")
            else:
                regions_file = load_package_resource(
                    "tide_model", "tide_regions_map.geojson"
                )
                logger.info(f"Using package resource for regions file at {regions_file}")
