...
def melt_df(df: pd.DataFrame, column_name: str) -> pd.DataFrame:
    ...
    elif "dates" not in df.columns:
        df = df.reset_index(names="dates")
    
    if "dates" in df.columns:
        df["dates"] = pd.to_datetime(df["dates"])
        # If the column_name already exists, we should exclude it from value_vars
        # otherwise melt will fail if column_name matches an existing column not in id_vars
        value_vars = [col for col in df.columns if col not in ["dates", "transect_id", column_name]]
        df = pd.melt(df, id_vars=["dates"], value_vars=value_vars, var_name="transect_id", value_name=column_name)
    return df
...
def read_content_csv(
    ...
    if any(df.columns.str.contains(r"(?i)month")):
        ...
        # FIX: Only melt if it's actually in matrix format (doesn't have transect_id or dates)
        if ("dates" not in df.columns and "transect_id" not in df.columns) or "Unnamed: 0" in df.columns:
...
def validate_tide_model_exists(location: str, model: str = "fes2022") -> bool:
    ...
    for a tide model, specifically looking for regional directories
    ...
    with open(tide_regions_file, "r") as f:
        regions_data = json.load(f)
    num_regions = len(regions_data["features"])

    # check if tide directory exists and if the model was clipped correctly to the regions
    if os.path.isdir(location) and contains_sub_directories(
        location, num_regions - 1, model
    ):
        return True
    return False
