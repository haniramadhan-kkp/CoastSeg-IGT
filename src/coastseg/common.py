...
def merge_tide_corrected_with_raw_timeseries(
    ...
    # Drop any columns from raw_timseries that are already in tide_timeseries (except dates and transect_id)
    cols_to_drop = [
        col
        for col in raw_timseries.columns
        if col in tide_timeseries.columns and col not in ["dates", "transect_id"]
    ]
    raw_timseries.drop(columns=cols_to_drop, inplace=True, errors="ignore")
...
def convert_transect_ids_to_rows(df):
    ...
    reshaped_df = df.melt(
        id_vars=id_vars, var_name="transect_id", value_name="cross_distance"
    )
...
def save_transects(
    ...
    # re-order columns
    columns_to_keep = ["dates", "x", "y", "transect_id", "cross_distance"]
    if "satname" in merged_timeseries_df.columns:
        columns_to_keep.append("satname")
    
    merged_timeseries_df = merged_timeseries_df[columns_to_keep]
...
def get_cross_distance_df(
    ...
    if "satname" in extracted_shorelines:
